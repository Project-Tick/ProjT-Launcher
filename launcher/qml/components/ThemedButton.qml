...
Button {
    ...
    readonly property color baseColor: {
        ...
        if (control.warning) return _warningColor
        if (control.primary) return _highlightColor
        return _buttonColor
    }
    ...
    background: Rectangle {
        ...
        color: {
            ...
            if (control.flatStyle) return control.hovered ? Qt.rgba(control.baseColor.r, control.baseColor.g, control.baseColor.b, 0.1) : "transparent"
            if (control.outline) return control.hovered ? Qt.rgba(control.baseColor.r, control.baseColor.g, control.baseColor.b, 0.1) : "transparent"
            
            var c = control.baseColor
            if (control.pressed) return Qt.darker(c, 1.2)
            if (control.hovered) return Qt.lighter(c, 1.1)
            return c
        }
        
        border.width: {
            if (control.outline) return 1.5
            if (control.visualFocus) return 2
            if (control.flatStyle) return 0
            return 1
        }
        
        border.color: {
            var _ = control._themeUpdateCount
            if (control.visualFocus) return control._highlightColor
            if (control.outline) {
                if (control.hovered) return Qt.lighter(control.baseColor, 1.2)
                return control.baseColor
            }
            if (control.flatStyle) return "transparent"
