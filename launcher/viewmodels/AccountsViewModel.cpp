...
AccountsViewModel::AccountsViewModel(QObject* parent) : QObject(parent)
{
    m_accountList = APPLICATION->accounts().get();

    if (m_accountList) {
        connect(m_accountList, &AccountList::listChanged, this, &AccountsViewModel::onListChanged);
        connect(m_accountList, &AccountList::defaultAccountChanged, this, &AccountsViewModel::onDefaultAccountChanged);
        connect(m_accountList, &AccountList::activityChanged, this, &AccountsViewModel::onActivityChanged);
    }
}
...
void AccountsViewModel::addMicrosoftAccount()
{
    ...
    if (m_currentLoginTask) {
        ...
        connect(m_currentLoginTask.get(), &AuthFlow::succeeded, this, [this, account]() {
            m_accountList->addAccount(account);
            m_currentLoginTask.reset();
            emit loginFinished(true, tr("Account added successfully"));
        });

        connect(m_currentLoginTask.get(), &AuthFlow::failed, this, [this](const QString& reason) {
            m_currentLoginTask.reset();
            emit loginFinished(false, reason);
        });

        // Handle OAuth URL for browser login
        connect(m_currentLoginTask.get(), &AuthFlow::authorizeWithBrowser, this,
                [this](const QUrl& url) { emit loginUrlReady(url.toString(), QString()); });
...
void AccountsViewModel::addMicrosoftAccountWithDevice()
{
    ...
    if (m_currentLoginTask) {
        ...
        connect(m_currentLoginTask.get(), &AuthFlow::authorizeWithBrowserWithExtra, this,
                [this](const QString& url, const QString& code, int) { emit loginUrlReady(url, code); });
...
{
    if (!m_accountList || username.isEmpty())
        return;
