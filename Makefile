...
LDFLAGS=
TEST_LIBS=-L. libz.a
LDSHARED=gcc -shared -Wl,-soname,libz.so.0,--version-script,zlib.map
CPP=
VGFMAFLAG=

STATICLIB=libz.a
SHAREDLIB=libz.so
SHAREDLIBV=libz.so.0.0.5.1
SHAREDLIBM=libz.so.0
...
zlib.5.pdf: $(SRCDIR)zlib.5
	groff -mandoc -f H -T ps $(SRCDIR)zlib.5 | ps2pdf - $@
