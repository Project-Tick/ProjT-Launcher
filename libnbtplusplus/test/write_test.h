...
class read_test : public CxxTest::TestSuite {
   public:
    ...
    void test_stream_writer_little()
    {
        ...
        TS_ASSERT(os);
        std::string expected{ 0x0d, 0x0c, 0x0b, 0x0a,  // 0x0a0b0c0d in Little Endian
    ...
    void test_write_payload_big()
    {
        ...
        TS_ASSERT_EQUALS(os.str(),
                         (std::string{ '\x7F', '\x7F', '\xFF', '\x7F', '\xFF', '\xFF', '\xFF', '\x7F', '\xFF', '\xFF', '\xFF', '\xFF',
                                       '\xFF', '\xFF', '\xFF',
        ...
        TS_ASSERT_EQUALS(os.str(), (std::string{ 0x00, 0x06,  // string length in Big Endian
                                                 'b', 'a', 'r', 'b', 'a', 'z' }));
        ...
        TS_ASSERT_EQUALS(os.str(), (std::string{ 0x00, 0x00, 0x00, 0x05,  // length in Big Endian
                                                 0, 1, 127, -128, -127 }));
        os.str("");

        // tag_int_array
        writer.write_payload(tag_int_array{ 0x01020304, 0x05060708, 0x090a0b0c });
        TS_ASSERT_EQUALS(os.str(), (std::string{ 0x00, 0x00, 0x00, 0x03,  // length in Big Endian
        ...
        writer.write_payload(tag_list{                  // nested list
                                       tag_list::of<tag_short>({ 0x3456, 0x789a }), tag_list::of<tag_byte>({ 0x0a, 0x0b, 0x0c, 0x0d }) });
        TS_ASSERT_EQUALS(os.str(), (std::string{ 0,                       // tag_type::End
        ...
        writer.write_payload(tag_compound{ { "foo", "quux" }, { "bar", tag_int(0x789abcde) } });
        ...
        TS_ASSERT(os.str() == endtag + subtag1 + subtag2 + endtag || os.str() == endtag + subtag2 + subtag1 + endtag);
