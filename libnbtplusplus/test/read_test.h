...
class read_test : public CxxTest::TestSuite {
   public:
    ...
    void verify_bigtest_structure(const tag_compound& comp)
    {
        ...
        TS_ASSERT(comp.at("byteArrayTest (the first 1000 values of (n*n*255+n*7)%100, starting with n=0 (0, 62, 34, 16, 8, ...))") ==
                  byteArrayTest);

        TS_ASSERT(comp.at("stringTest") == tag_string("HELLO WORLD THIS IS A TEST STRING \u00C5\u00C4\u00D6!"));

        TS_ASSERT(comp.at("listTest (compound)") ==
                  tag_list::of<tag_compound>({ { { "created-on", tag_long(1264099775885) }, { "name", "Compound tag #0" } },
        ...
        TS_ASSERT((comp.at("nested compound test") == tag_compound{ { "egg", tag_compound{ { "value", 0.5f }, { "name", "Eggbert" } } },
                                                                    { "ham", tag_compound{ { "value", 0.75f }, { "name", "Hampus" } } } }));

        TS_ASSERT(comp.at("intArrayTest") == tag_int_array({ 0x00010203, 0x04050607, 0x08090a0b, 0x0c0d0e0f }));
    }
    ...
    void test_read_misc()
    {
        ...
        TS_ASSERT(*pair.second == tag_string("Even though unprovided for by NBT, the library should also handle "
