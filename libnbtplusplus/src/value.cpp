...
void assign_numeric_impl(std::unique_ptr<tag>& tag_ptr, T val, tag_type default_type)
{
    ...
    switch (existing_type) {
        case tag_type::Byte:
            static_cast<tag_byte&>(*tag_ptr).set(static_cast<int8_t>(val));
            break;
        case tag_type::Short:
            static_cast<tag_short&>(*tag_ptr).set(static_cast<int16_t>(val));
            break;
        case tag_type::Int:
            static_cast<tag_int&>(*tag_ptr).set(static_cast<int32_t>(val));
            break;
        case tag_type::Long:
            static_cast<tag_long&>(*tag_ptr).set(static_cast<int64_t>(val));
            break;
        case tag_type::Float:
            static_cast<tag_float&>(*tag_ptr).set(static_cast<float>(val));
            break;
        case tag_type::Double:
            static_cast<tag_double&>(*tag_ptr).set(static_cast<double>(val));
            break;
        default:
            throw std::bad_cast();
    }
