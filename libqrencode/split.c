    ...
    while (isalnum(*p)) {
        if (isdigit(*p)) {
            ...
            dif = QRinput_estimateBitsModeAn((int)(p - string)) /* + 4 + la */
                  + QRinput_estimateBitsModeNum((int)(q - p)) + 4 + ln + (isalnum(*q) ? (4 + ln) : 0) -
    ...
    while (*p != '\0') {
        ...
        if (mode == QR_MODE_NUM) {
            ...
            dif = QRinput_estimateBitsMode8((int)(p - string)) /* + 4 + l8 */
                  + QRinput_estimateBitsModeNum((int)(q - p)) + 4 + ln + swcost -
            ...
            dif = QRinput_estimateBitsMode8((int)(p - string)) /* + 4 + l8 */
                  + QRinput_estimateBitsModeAn((int)(q - p)) + 4 + la + swcost -
                  QRinput_estimateBitsMode8((int)(q - string)) /* - 4 - l8 */;
            if (dif < 0) {
                break;
            }
