...
- **FIXME** in `launcher\Application.h`:355 — // FIXME: attach to instances instead.
```
#endif
...
- **TODO** in `website\projtlauncher\wiki\development\instructions\windows.md`:124 — <!-- TODO: The VS-Code instructions could be done better for windows (and in general) -->
```
...
- **FIXME** in `launcher\minecraft\ComponentUpdateTask.cpp`:180 — // FIXME: to do this right, we need to load the lists and decide on which versions to use during dependency
```
...
- **FIXME** in `launcher\ui\pages\instance\LogPage.cpp`:294 — // FIXME: turn this into a proper task and move the upload logic out of GuiUtil!
```
		return;
...
- **todo** in `launcher\modplatform\atlauncher\ATLPackInstallTask.cpp`:1240 — // todo: use more detection
```
...
- **FIXME** in `launcher\minecraft\launch\LauncherPartLaunch.cpp`:237 — // FIXME: make this work again
```
				return;
			}
...
- **TODO** in `launcher\java\download\ManifestDownloadTask.cpp`:142 — // TODO: Implement decompression support for lzma/lz4 to enable faster downloads
```
...
- **FIXME** in `launcher\CMakeLists.txt`:1502 — # FIXME: HACK: Fix manifest issues with Ninja in release mode (and only release mode) and MSVC
```
...
- **TODO** in `libqrencode\ChangeLog`:305 — * ChangeLog, NEWS, TODO:
```
