...
- **FIXME** in `.markdownlintignore`:9 — <!-- FIXME: HACK -->
```
...
- **FIXME** in `.markdownlintignore`:14 — <!-- FIXME: HACK -->
```
cmark
extra-cmake-modules
<!-- FIXME: HACK -->
...
- **FIXME** in `launcher\net\NetRequest.cpp`:246 — * FIXME: report Qt bug for this
```
        /*
...
- **TODO** in `launcher\java\download\ManifestDownloadTask.cpp`:123 — // TODO: Implement decompression support for lzma/lz4 to enable faster downloads
```
...
- **FIXME** in `launcher\ui\pages\instance\ScreenshotsPage.cpp`:205 — // FIXME: this is a workaround for a bug in QFileSystemModel, where it doesn't
```
        if (role != Qt::EditRole)
            return false;
...
- **todo** in `launcher\ui\widgets\FileIgnoreProxy.cpp`:182 — QStack<QModelIndex> todo;
```
...
- **todo** in `launcher\ui\widgets\FileIgnoreProxy.cpp`:186 — if (!todo.size()) {
```
                auto node = fsm->index(row, 0, doing);
                if (!node.isValid()) {
                    if (!todo.size()) {
...
- **todo** in `launcher\ui\widgets\FileIgnoreProxy.cpp`:189 — doing = todo.pop();
```
                        break;
                    } else {
                        doing = todo.pop();
...
- **todo** in `launcher\ui\widgets\FileIgnoreProxy.cpp`:198 — todo.push(node);
```
                {
...
- **todo** in `launcher\ui\widgets\FileIgnoreProxy.cpp`:222 — QStack<QModelIndex> todo;
```
...
- **todo** in `launcher\ui\widgets\FileIgnoreProxy.cpp`:226 — if (!todo.size()) {
```
            auto node = this->index(row, 0, doing);
            if (!node.isValid()) {
                if (!todo.size()) {
...
- **todo** in `launcher\ui\widgets\FileIgnoreProxy.cpp`:229 — doing = todo.pop();
```
                    break;
                } else {
                    doing = todo.pop();
...
- **todo** in `launcher\ui\widgets\FileIgnoreProxy.cpp`:235 — todo.push(node);
```
            }
