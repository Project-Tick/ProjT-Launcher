    ...
    static constexpr auto check_value = [](const auto& init_value, auto expected_native_type_tag) {
        ...
        if constexpr (std::is_same_v<std::string, native_type>) {
            ...
            if constexpr (impl::is_one_of<init_char_type, char, char8_type>) {
                const auto coerced_view = std::string_view{ reinterpret_cast<const char*>(init_view.data()), init_view.length() };
        ...
        CHECK(v.is_number() == (expected_node_type == node_type::integer || expected_node_type == node_type::floating_point));
