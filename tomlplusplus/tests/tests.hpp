...
class function_view<R(P...)> final {
   ...
   public:
    ...
    decltype(auto) operator()(P&&... xs) const { return eraser(ptr_, std::forward<P>(xs)...); }
...
template <typename T>
inline bool parse_expected_value(std::string_view test_file, uint32_t test_line, std::string_view value_str, const T& expected)
{
    ...
    value<value_type> val_parsed;
    {
        ...
        const auto result = parsing_should_succeed(test_file, test_line, std::string_view{ val }, [&](table&& tbl) {
            REQUIRE(tbl.size() == 1);
            auto nv = tbl["val"sv];
            REQUIRE(nv);
    ...
    // check round-tripping
    {
                   ...
                   << "parse_expected_value: Checking round-trip"sv)
        {
            ...
            bool value_ok = true;
            const auto parse_ok = parsing_should_succeed(test_file, test_line, std::string_view{ str }, [&](table&& tbl) {
                REQUIRE(tbl.size() == 1);
                auto nv = tbl["val"sv];
                REQUIRE(nv);
                REQUIRE(nv.as<value_type>());
