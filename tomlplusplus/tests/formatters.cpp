...
static auto format_to_string(const T& obj, format_flags flags = Formatter::default_flags, format_flags exclude_flags = format_flags::none)
{
    std::stringstream ss;
...
TEST_CASE("formatters")
{
    ...
    const auto data = toml::table{
        { "integers"sv, toml::table{ { "zero"sv, 0 },
                                     { "one"sv, 1 },
                                     { "dec"sv, 10 },
        ...
        { "floats"sv, toml::table{ { "pos_zero"sv, +0.0 },
                                   { "neg_zero"sv, -0.0 },
                                   { "one"sv, 1.0 },
          ...
          toml::table{
              ...
              { "date-times"sv,
                toml::table{
                    ...
                    { "offset"sv, toml::table{ { "val"sv, toml::date_time{ data_date, data_time, toml::time_offset{} } } } } } } } },

        { "bools"sv, toml::table{ { "true"sv, true },  //
                                  { "false"sv, false } } },

        {
            "strings"sv,
            toml::array{ R"()"sv, R"(string)"sv, R"(string with a single quote in it: ')"sv, R"(string with a double quote in it: ")"sv,
                         "string with a tab: \t"sv, R"(a long string to force the array over multiple lines)"sv },
        },

        { "a"sv, toml::table{ { "val", true }, { "b"sv, toml::table{ { "val", true }, { "c"sv, toml::table{ { "val", true } } } } } } }
