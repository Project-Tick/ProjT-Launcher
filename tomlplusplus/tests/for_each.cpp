...
TEST_CASE("array::for_each")
{
    ...
    static_cast<array&>(arr).for_each([](auto&& elem, size_t) noexcept {
    ...
    static_cast<array&>(arr).for_each([](size_t, auto&& elem) noexcept {
        using elem_ref_type = decltype(elem);
        static_assert(std::is_lvalue_reference_v<elem_ref_type>);
    ...
    static_cast<array&>(arr).for_each([](auto&& elem) noexcept {
        using elem_ref_type = decltype(elem);
        static_assert(std::is_lvalue_reference_v<elem_ref_type>);
    ...
    static_cast<const array&>(arr).for_each([](auto&& elem, size_t) noexcept {
        using elem_ref_type = decltype(elem);
        static_assert(std::is_lvalue_reference_v<elem_ref_type>);
    ...
    static_cast<const array&>(arr).for_each([](size_t, auto&& elem) noexcept {
        using elem_ref_type = decltype(elem);
        static_assert(std::is_lvalue_reference_v<elem_ref_type>);
    ...
    static_cast<const array&>(arr).for_each([](auto&& elem) noexcept {
        using elem_ref_type = decltype(elem);
        static_assert(std::is_lvalue_reference_v<elem_ref_type>);
...
TEST_CASE("table::for_each")
{
    ...
    // check lvalue propagates correctly
    static_cast<table&>(tbl).for_each([](const toml::key&, auto&& elem) noexcept {
        using elem_ref_type = decltype(elem);
        static_assert(std::is_lvalue_reference_v<elem_ref_type>);
    ...
    static_cast<table&>(tbl).for_each([](auto&& elem) noexcept {
        using elem_ref_type = decltype(elem);
        static_assert(std::is_lvalue_reference_v<elem_ref_type>);
    ...
    // check const lvalue propagates correctly
    static_cast<const table&>(tbl).for_each([](const toml::key&, auto&& elem) noexcept {
        using elem_ref_type = decltype(elem);
        static_assert(std::is_lvalue_reference_v<elem_ref_type>);
    ...
    static_cast<const table&>(tbl).for_each([](auto&& elem) noexcept {
        using elem_ref_type = decltype(elem);
        static_assert(std::is_lvalue_reference_v<elem_ref_type>);
    ...
    // check that early-stopping works
    {
        toml::table tbl2;
        size_t added{};
        tbl.for_each([&](auto&& key, const auto& val) {
