...
number=5)"sv;
    parsing_should_succeed(FILE_LINE_ARGS, data, [](auto&& tbl) {
        ...
        auto count_table_lambda = [tbl_begin, tbl_end](node_type type) noexcept {
            return std::count_if(tbl_begin, tbl_end, [type](const auto& pair) noexcept { return pair.second.type() == type; });
        };
        ...
        const auto arr_iter = std::find_if(tbl_begin, tbl_end, [](const auto& pair) noexcept { return pair.second.is_array(); });
        ...
        auto count_array_lambda = [arr_begin, arr_end](node_type type) noexcept {
            return std::count_if(arr_begin, arr_end, [type](const auto& node) noexcept { return node.type() == type; });
        };
