...
TEST_CASE("tables - moving")
{
    ...
    parsing_should_succeed(
        FILE_LINE_ARGS, R"(test = { val1 = "foo" })"sv,
        [&](table&& tbl) {
...
TEST_CASE("tables - construction")
{
    ...
    {
        table tbl{ { L"foo", L"test1" }, { L"bar"sv, L"test2"sv }, { L"kek"s, L"test3"sv }, { L"qux"sv.data(), L"test4"sv.data() } };
        CHECK(tbl.size() == 4u);
        CHECK(!tbl.empty());
...
{
    static constexpr auto to_string = [](std::string_view some_toml, format_flags flags = toml_formatter::default_flags,
...
key8 = [
    ...
    'i guess thats what tests are for'
        ...
        CHECK(to_string(input, toml_formatter::default_flags, format_flags::indent_sub_tables) == expected_without_indented_subtables);
...
key5 = 'val'
    ...
    'i guess thats what tests are for'
        ...
        CHECK(to_string(input, toml_formatter::default_flags, format_flags::indent_array_elements) == expected_without_indented_arrays);
...
'i guess thats what tests are for'
        ...
        CHECK(to_string(input, toml_formatter::default_flags, format_flags::indentation) == expected_without_indentation);
...
key5 = 'val'
    ...
    key8 = [
        ...
        CHECK(to_string(input, toml_formatter::default_flags | format_flags::force_multiline_arrays) == expected_forced_multiline);
...
key5 = 'val'
    ...
    'i guess thats what tests are for'
        ...
        CHECK(to_string(input, toml_formatter::default_flags | format_flags::force_multiline_arrays, format_flags::indent_array_elements) ==
...
TEST_CASE("tables - for_each")
{
    ...
    SECTION("type checking")
    {
        ...
        int strings = 0;
        int bools = 0;
        tbl.for_each([&](const auto& v) noexcept {
            count++;
