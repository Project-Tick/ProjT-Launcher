...
void QuaZip::close()
{
    ...
    switch (p->mode) {
        ...
        case mdAppend:
        case mdAdd:
            p->zipError = zipClose(p->zipFile_f, p->comment.isNull() ? nullptr
...
QString QuaZip::getComment() const
{
    ...
    fakeThis->p->zipError = UNZ_OK;
    unsigned flags = 0;
    return (unzGetFileFlags(p->unzFile_f, &flags) == UNZ_OK) && (flags & UNZ_ENCODING_UTF8) ? QString::fromUtf8(comment)
...
bool QuaZip::getCurrentFileInfo(QuaZipFileInfo64* info) const
{
    ...
    if ((fakeThis->p->zipError = unzGetCurrentFileInfo64(p->unzFile_f, nullptr, fileName.data(), fileName.size(), extra.data(),
...
QString QuaZip::getCurrentFileName() const
{
    ...
    if ((fakeThis->p->zipError =
             unzGetCurrentFileInfo64(p->unzFile_f, &file_info, fileName.data(), fileName.size(), nullptr, 0, nullptr, 0)) != UNZ_OK)
        return QString();
