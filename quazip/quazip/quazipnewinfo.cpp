...
QuaZipNewInfo::QuaZipNewInfo(const QString& _name)
    : name(_name), dateTime(QDateTime::currentDateTime()), internalAttr(0), externalAttr(0), uncompressedSize(0)
{}

QuaZipNewInfo::QuaZipNewInfo(const QString& _name, const QString& file) : name(_name), internalAttr(0), externalAttr(0), uncompressedSize(0)
{
    QFileInfo info(file);
...
static void setNTFSTime(QByteArray& extra, const QDateTime& time, int position, int fineTicks)
{
    ...
    if (ntfsTimesLength < 24) {
        ...
        memmove(extra.data() + timesEnd + (24 - ntfsTimesLength), extra.data() + timesEnd,
