...
class QuaZipDirPrivate : public QSharedData {
   ...
   private:
    ...
    bool entryInfoList(QStringList nameFilters, QDir::Filters filter, QDir::SortFlags sort, TFileInfoList& result) const;
...
bool QuaZipDirComparator::operator()(const QuaZipFileInfo64& info1, const QuaZipFileInfo64& info2)
{
    ...
    switch (order) {
        ...
        case QDir::Time:
            ...
            break;
        default:
            qWarning("QuaZipDirComparator(): Invalid sort mode 0x%2X", static_cast<unsigned>(sort));
            return false;
    }
...
template <typename TFileInfoList>
bool QuaZipDirPrivate::entryInfoList(QStringList _nameFilters, QDir::Filters _filter, QDir::SortFlags sort, TFileInfoList& result) const
{
...
QList<QuaZipFileInfo> QuaZipDir::entryInfoList(const QStringList& nameFilters, QDir::Filters filters, QDir::SortFlags sort) const
{
...
QList<QuaZipFileInfo64> QuaZipDir::entryInfoList64(const QStringList& nameFilters, QDir::Filters filters, QDir::SortFlags sort) const
{
...
bool QuaZipDir::exists(const QString& filePath) const
{
    ...
    if (fileName.contains(QLatin1String("/"))) {
        ...
        qDebug(
            ...
            "fileInfo.path()=%s",
            fileName.toUtf8().constData(), fileInfo.fileName().toUtf8().constData(), fileInfo.path().toUtf8().constData());
#endif
        QuaZipDir dir(*this);
