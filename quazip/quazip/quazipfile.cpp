...
class QuaZipFilePrivate {
   ...
   private:
    ...
    inline QuaZipFilePrivate(QuaZipFile* _q, const QString& zipName)
        : q(_q), caseSensitivity(QuaZip::csDefault), raw(false), writePos(0), uncompressedSize(0), crc(0), internal(true), zipError(UNZ_OK)
    {
        zip = new QuaZip(zipName);
    }
    /// The constructor for the corresponding QuaZipFile constructor.
    inline QuaZipFilePrivate(QuaZipFile* _q, const QString& zipName, const QString& _fileName, QuaZip::CaseSensitivity cs)
        : q(_q), raw(false), writePos(0), uncompressedSize(0), crc(0), internal(true), zipError(UNZ_OK)
    {
        zip = new QuaZip(zipName);
    ...
    inline QuaZipFilePrivate(QuaZipFile* _q, QuaZip* _zip)
        : q(_q), zip(_zip), raw(false), writePos(0), uncompressedSize(0), crc(0), internal(false), zipError(UNZ_OK)
    {}
    /// The destructor.
...
QuaZipFile::QuaZipFile(const QString& zipName, QObject* parent) : QIODevice(parent), p(new QuaZipFilePrivate(this, zipName)) {}
    ...
    if ((mode & ReadOnly) && !(mode & WriteOnly)) {
        if (p->internal) {
            ...
            if (p->zip->getMode() != QuaZip::mdUnzip) {
                qWarning("QuaZipFile::open(): file open mode %d incompatible with ZIP open mode %d", (int)mode,
                      ...
                      int strategy)
{
    ...
    if ((mode & WriteOnly) && !(mode & ReadOnly)) {
        ...
        p->setZipError(zipOpenNewFileInZip4_64(
            p->zip->getZipFile(),
            p->zip->isUtf8Enabled() ? info.name.toUtf8().constData() : p->zip->getFileNameCodec()->fromUnicode(info.name).constData(),
            &info_z, info.extraLocal.constData(), info.extraLocal.length(), info.extraGlobal.constData(), info.extraGlobal.length(),
            p->zip->isUtf8Enabled() ? info.comment.toUtf8().constData() : p->zip->getCommentCodec()->fromUnicode(info.comment).constData(),
            method, level, static_cast<int>(raw), windowBits, memLevel, strategy, password, static_cast<uLong>(crc),
            (p->zip->getOsCode() << 8) | QUAZIP_VERSION_MADE_BY, 0, p->zip->isZip64Enabled()));
        if (p->zipError != UNZ_OK) {
            return false;
        }
        p->writePos = 0;
