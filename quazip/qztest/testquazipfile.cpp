...
void TestQuaZipFile::zipUnzip_data()
{
    ...
    QTest::newRow("Unicode") << "unicode.zip"
                             << (QStringList() << QString::fromUtf8("Українське сало.txt") << QString::fromUtf8("Vin français.txt")
    ...
    QTest::newRow("password") << "password.zip" << (QStringList() << "test.txt") << QByteArray() << QByteArray("PassPass") << Z_DEFLATED
                              << Z_DEFAULT_COMPRESSION << false << false << -1;
    QTest::newRow("zip64") << "zip64.zip" << (QStringList() << "test64.txt") << QByteArray() << QByteArray() << Z_DEFLATED
                           << Z_DEFAULT_COMPRESSION << true << false << -1;
    QTest::newRow("large enough to flush") << "flush.zip" << (QStringList() << "flush.txt") << QByteArray() << QByteArray() << Z_DEFLATED
...
void TestQuaZipFile::zipUnzip()
{
    ...
    foreach (QString fileName, fileNames) {
        ...
        if (!outFile.open(QIODevice::WriteOnly, QuaZipNewInfo(fileName, inFile.fileName()),
                          password.isEmpty() ? NULL : password.constData(), 0, method, level)) {
        ...
        for (qint64 pos = 0, len = inFile.size(); pos < len;) {
            ...
            if ((l = inFile.read(buf, readSize)) != readSize) {
                qDebug("Reading %ld bytes from %s at %ld returned %ld", static_cast<long>(readSize), fileName.toUtf8().constData(),
    ...
    if (!password.isEmpty()) {
        ...
        QVERIFY(readError != ZIP_OK || archivedData.isEmpty() ||
                archivedData != originalData);  // corrupt data or empty, depends on the platform
    }
    testUnzip.close();
...
void TestQuaZipFile::zipUnzipLarge()
{
    ...
    QuaZipFile outFile(&testZip);
    if (!outFile.open(QIODevice::WriteOnly, QuaZipNewInfo(fileName, inFile.fileName()), NULL, 0, Z_DEFLATED,
...
void TestQuaZipFile::setFileAttrs()
{
    ...
    }
    {
        ...
        QFile::Permissions usedPermissions = QFile::WriteOwner | QFile::ReadOwner | QFile::ExeOwner | QFile::WriteGroup | QFile::ReadGroup |
                                             QFile::ExeGroup | QFile::WriteOther | QFile::ReadOther | QFile::ExeOther;
