...
void TestJlCompress::compressFileOptions_data()
{
    ...
    QTest::newRow("simple") << "jlsimplefile.zip"
                            << "test0.txt" << QDateTime(QDate(2024, 9, 19), QTime(21, 0, 0), COMPAT_UTC_TZ) << JlCompress::Options::Default
    ...
    QTest::newRow("simple-storage") << "jlsimplefile-storage.zip"
                                    << "test0.txt" << QDateTime(QDate(2024, 9, 19), QTime(21, 0, 0), COMPAT_UTC_TZ)
    ...
    QTest::newRow("simple-standard") << "jlsimplefile-standard.zip"
                                     << "test0.txt" << QDateTime(QDate(2024, 9, 19), QTime(21, 0, 0), COMPAT_UTC_TZ)
                                     << JlCompress::Options::Standard << "5eedd83aee92cf3381155d167fee54a4ef6e43b8bc7a979c903611d9aa28610a"
                                     << "752db50b15db1a19e091f9c1b43ec22b279867b20d43c76bc9a01d7bc0d7ae4f"
                                     << "cb1847dff1a5c33a805efde2558fc74024ad4c64c8607f8b12903e4d92385955";
...
void TestJlCompress::compressFiles_data()
{
    ...
    QTest::newRow("simple") << "jlsimplefiles.zip" << (QStringList() << "test0.txt" << "test00.txt");
    QTest::newRow("different subdirs") << "jlsubdirfiles.zip" << (QStringList() << "subdir1/test1.txt" << "subdir2/test2.txt");
}
...
void TestJlCompress::compressDir_data()
{
    ...
    QTest::newRow("empty dirs") << "jldir_empty.zip" << (QStringList() << "testdir1/" << "testdir2/testdir3/")
                                << (QStringList() << "testdir1/" << "testdir2/"
                                                  << "testdir2/testdir3/");
    QTest::newRow("hidden files") << "jldir_hidden.zip" << (QStringList() << ".test0.txt" << "test1.txt")
...
void TestJlCompress::compressDir()
{
    ...
    for (int i = 0; i < fileNames.size(); ++i) {
        if (fileNames.at(i).startsWith(".")) {
            ...
            SetFileAttributesW(reinterpret_cast<LPCWSTR>(fn.utf16()), FILE_ATTRIBUTE_HIDDEN);
        }
    }
#endif
...
void TestJlCompress::compressDirOptions_data()
{
    ...
    QTest::newRow("simple") << "jldir.zip"
                            ...
                            << (QStringList() << "test0.txt"
                                              ...
                                              << "testdir2/" << "testdir2/test2.txt"
                                              << "testdir2/subdir/" << "testdir2/subdir/test2sub.txt")
                            << QDateTime(QDate(2024, 9, 19), QTime(21, 0, 0), COMPAT_UTC_TZ) << JlCompress::Options::Default
    ...
    QTest::newRow("simple-storage") << "jldir-storage.zip"
                                    ...
                                    << (QStringList() << "test0.txt"
                                                      ...
                                                      << "testdir2/" << "testdir2/test2.txt"
                                                      << "testdir2/subdir/" << "testdir2/subdir/test2sub.txt")
                                    << QDateTime(QDate(2024, 9, 19), QTime(21, 0, 0), COMPAT_UTC_TZ) << JlCompress::Options::Storage << ""
                                    << ""
                                    << "";
    QTest::newRow("simple-fastest") << "jldir-fastest.zip"
                                    ...
                                    << (QStringList() << "test0.txt"
                                                      ...
                                                      << "testdir2/" << "testdir2/test2.txt"
                                                      << "testdir2/subdir/" << "testdir2/subdir/test2sub.txt")
                                    << QDateTime(QDate(2024, 9, 19), QTime(21, 0, 0), COMPAT_UTC_TZ) << JlCompress::Options::Fastest << ""
                                    << ""
                                    << "";
    QTest::newRow("simple-faster") << "jldir-faster.zip"
                                   ...
                                   << (QStringList() << "test0.txt"
                                                     ...
                                                     << "testdir2/" << "testdir2/test2.txt"
                                                     << "testdir2/subdir/" << "testdir2/subdir/test2sub.txt")
                                   << QDateTime(QDate(2024, 9, 19), QTime(21, 0, 0), COMPAT_UTC_TZ) << JlCompress::Options::Faster << ""
                                   << ""
                                   << "";
    QTest::newRow("simple-standard") << "jldir-standard.zip"
                                     ...
                                     << (QStringList() << "test0.txt"
                                                       ...
                                                       << "testdir2/" << "testdir2/test2.txt"
                                                       << "testdir2/subdir/" << "testdir2/subdir/test2sub.txt")
                                     << QDateTime(QDate(2024, 9, 19), QTime(21, 0, 0), COMPAT_UTC_TZ) << JlCompress::Options::Standard
                                     << "ed0d5921b2fc11b6b4cb214b3e43ea3ea28987d6ff8080faab54c4756de30af6"
                                     << "299cd566069754a4ca1deb025e279be3cca80e454132b51fa2a22e41c8ef1299"
                                     << "1eba110a33718c07a4ddf3fa515d1b4c6e3f4fc912b2e29e5e32783e2cddf852";
    QTest::newRow("simple-better") << "jldir-better.zip"
                                   ...
                                   << (QStringList() << "test0.txt"
                                                     ...
                                                     << "testdir2/" << "testdir2/test2.txt"
                                                     << "testdir2/subdir/" << "testdir2/subdir/test2sub.txt")
                                   << QDateTime(QDate(2024, 9, 19), QTime(21, 0, 0), COMPAT_UTC_TZ) << JlCompress::Options::Better << ""
                                   << ""
                                   << "";
    QTest::newRow("simple-best") << "jldir-best.zip"
                                 ...
                                 << (QStringList() << "test0.txt"
                                                   ...
                                                   << "testdir2/" << "testdir2/test2.txt"
                                                   << "testdir2/subdir/" << "testdir2/subdir/test2sub.txt")
                                 << QDateTime(QDate(2024, 9, 19), QTime(21, 0, 0), COMPAT_UTC_TZ) << JlCompress::Options::Best << ""
                                 << ""
                                 << "";
}
...
void TestJlCompress::extractFile_data()
{
    ...
    QTest::newRow("russian") << "jlextfilerus.zip"
                             << (QStringList() << "test0.txt" << "testdir1/test1.txt" << QString::fromUtf8("testdir2/тест2.txt")
                                               << "testdir2/subdir/test2sub.txt")
                             << QString::fromUtf8("testdir2/тест2.txt") << QString::fromUtf8("тест2.txt") << QByteArray("IBM866");
    QTest::newRow("extract dir") << "jlextdir.zip" << (QStringList() << "testdir1/") << "testdir1/" << "testdir1/" << QByteArray();
}
...
void TestJlCompress::extractFile()
{
    ...
    QVERIFY(!JlCompress::extractFile(zipName, fileToExtract, "jlext/jlfile/" + destName).isEmpty());
    ...
    QFile _zipFile(zipName);
    QVERIFY(_zipFile.open(QIODevice::ReadOnly));
    QVERIFY(!JlCompress::extractFile(&_zipFile, fileToExtract, "jlext/jlfile/" + destName).isEmpty());
...
void TestJlCompress::extractFiles()
{
    ...
    QVERIFY(!JlCompress::extractFiles(zipName, filesToExtract, "jlext/jlfiles").isEmpty());
    ...
    QFile _zipFile(zipName);
    QVERIFY(_zipFile.open(QIODevice::ReadOnly));
    QVERIFY(!JlCompress::extractFiles(&_zipFile, filesToExtract, "jlext/jlfiles").isEmpty());
    foreach (QString fileName, filesToExtract) {
        QFileInfo fileInfo("jlext/jlfiles/" + fileName);
        QFileInfo extInfo("tmp/" + fileName);
        QCOMPARE(fileInfo.size(), extInfo.size());
...
void TestJlCompress::extractDir_data()
{
    ...
    QTest::newRow("simple") << "jlextdir.zip"
                            ...
                            << (QStringList() << "test0.txt" << "testdir1/test1.txt"
                                              << "testdir2/test2.txt" << "testdir2/subdir/test2sub.txt")
                            << "tmp/jlext/jldir/" << QByteArray();
    QTest::newRow("separate dir") << "sepdir.zip" << (QStringList() << "laj/" << "laj/lajfile.txt")
                                  << (QStringList() << "laj/" << "laj/lajfile.txt") << "tmp/jlext/jldir/" << QByteArray();
    QTest::newRow("Zip Slip") << "zipslip.zip" << (QStringList() << "test0.txt" << "../zipslip.txt") << (QStringList() << "test0.txt")
                              << "tmp/jlext/jldir/" << QByteArray();
    QTest::newRow("Cyrillic") << "cyrillic.zip" << (QStringList() << QString::fromUtf8("Ще не вмерла Україна"))
                              << (QStringList() << QString::fromUtf8("Ще не вмерла Україна")) << "tmp/jlext/jldir/" << QByteArray("KOI8-U");
    QTest::newRow("Japaneses") << "japanese.zip" << (QStringList() << QString::fromUtf8("日本"))
                               << (QStringList() << QString::fromUtf8("日本")) << "tmp/jlext/jldir/" << QByteArray("UTF-8");
#ifdef QUAZIP_EXTRACT_TO_ROOT_TEST
    QTest::newRow("Exract to root") << "tmp/quazip-root-test.zip" << (QStringList() << "tmp/quazip-root-test/test.txt")
                                    << (QStringList() << "tmp/quazip-root-test/test.txt") << QString() << QByteArray("UTF-8");
#endif
}
...
void TestJlCompress::addFileOptions_data()
{
    ...
    QTest::newRow("simple") << "jladdfileoptions.zip"
                            << "test0.txt"
                            << "test1.txt" << QDateTime(QDate(2024, 9, 19), QTime(21, 0, 0), COMPAT_UTC_TZ) << JlCompress::Options::Default;
}
...
void TestJlCompress::addFiles_data()
{
    ...
    QTest::newRow("simple") << "jladdfiles.zip"
                            << "test0.txt" << (QStringList() << "test1.txt" << "test2.txt");
    QTest::newRow("subdirs") << "jladdfiles-subdirs.zip"
                             << "test0.txt" << (QStringList() << "subdir1/test1.txt" << "subdir2/test2.txt");
}
...
void TestJlCompress::addFilesOptions()
{
    ...
    const JlCompress::Options defaultOptions(QDateTime(QDate(2024, 9, 19), QTime(21, 0, 0), COMPAT_UTC_TZ), JlCompress::Options::Default);
    QVERIFY(JlCompress::compressFile(zipName, "tmp/test0.txt", defaultOptions));

    // Add batch 1 (2 files) with Storage compression
    const JlCompress::Options storageOptions(QDateTime(QDate(2024, 9, 20), QTime(12, 0, 0), COMPAT_UTC_TZ), JlCompress::Options::Storage);
    QStringList batch1;
    ...
    const JlCompress::Options bestOptions(QDateTime(QDate(2024, 9, 21), QTime(15, 30, 0), COMPAT_UTC_TZ), JlCompress::Options::Best);
    QStringList batch2;
    ...
    const JlCompress::Options fastestOptions(QDateTime(QDate(2024, 9, 22), QTime(18, 0, 0), COMPAT_UTC_TZ), JlCompress::Options::Fastest);
    QStringList batch3;
...
void TestJlCompress::addDirOptions_data()
{
    ...
    QTest::newRow("simple") << "jladddir-options.zip"
                            << "test0.txt" << (QStringList() << "addtest1.txt" << "adddir1/addtest2.txt")
                            << (QStringList() << "addtest1.txt"
                                              << "adddir1/" << "adddir1/addtest2.txt")
                            << QDateTime(QDate(2024, 9, 19), QTime(21, 0, 0), COMPAT_UTC_TZ) << JlCompress::Options::Default;
    QTest::newRow("empty dir") << "jladddir-empty.zip"
                               << "test0.txt" << (QStringList() << "empty1/" << "parent/empty2/")
