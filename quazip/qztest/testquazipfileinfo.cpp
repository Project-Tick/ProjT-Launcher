...
void TestQuaZipFileInfo::getExtTime_data()
{
    ...
    QTest::newRow("no times") << QString::fromUtf8("noTimes") << static_cast<quint8>(0) << static_cast<quint16>(1) << QList<qint32>()
                              << static_cast<quint16>(1) << QList<qint32>() << QDateTime() << QDateTime() << QDateTime();
    ...
    QTest::newRow("no ac time") << QString::fromUtf8("noAcTime") << static_cast<quint8>(5) << static_cast<quint16>(9)
                                ...
                                << QDateTime(QDate(1970, 1, 1), QTime(0, 0, 3), COMPAT_UTC_TZ);
    QTest::newRow("negativeTime") << QString::fromUtf8("negativeTime") << static_cast<quint8>(1) << static_cast<quint16>(5)
                                  << (QList<qint32>() << -1) << static_cast<quint16>(5) << (QList<qint32>() << -1)
                                  << QDateTime(QDate(1969, 12, 31), QTime(23, 59, 59), COMPAT_UTC_TZ) << QDateTime() << QDateTime();
}
...
void TestQuaZipFileInfo::parseExtraField_data()
{
    ...
    }
    {
        ...
        QTest::newRow("one zero ID with data") << QByteArray(
                                                      "\x00\x00\x04\x00"
                                                      "DATA",
                                                      8)
                                               << oneZeroIdWithData;
    }
    {
        ...
        QTest::newRow("one non zero ID with data") << QByteArray(
                                                          "\x01\x00\x04\x00"
                                                          "DATA",
                                                          8)
    ...
    }
    {
        ...
        QTest::newRow("two IDs") << QByteArray(
                                        "\x00\x00\x05\x00"
                                        "DATA0"
                                        "\x01\x00\x05\x00"
                                        "DATA1",
    ...
    }
    {
        ...
        QTest::newRow("repeated ID") << QByteArray(
                                            "\x00\x00\x05\x00"
                                            "DATA0"
                                            "\x00\x00\x05\x00"
                                            "DATA1",
    ...
    }
    {
        ...
        QTest::newRow("large ID") << QByteArray(
                                         "\x02\x01\x04\x00"
                                         "DATA",
                                         8)
                                  << largeID;
